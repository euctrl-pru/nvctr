...
Compute the along-track distances of a body, \`b\` (for example a ground
level projection position of an aircraft), from two geographical
coordinates, \`a1\` and \`a2\` (for example an airport's runway
thresholds), of a great circle arc.
...
the surface along-track distances from \`b\`'s cross-track intersection
to \`a1\` - \`a2\`
